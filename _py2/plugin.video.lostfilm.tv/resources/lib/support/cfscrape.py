...
DEFAULT_USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.116 Safari/537.36"
...
class CloudflareScraper(Session):
    ...
    def solve_cf_challenge(self, resp, **original_kwargs):
        ...
        submit_url = "%s://%s" % (parsed_url.scheme, domain)

        form_submit = re.search('id="challenge-form" action="(.*?)"', body).group(1)
        if form_submit:
            submit_url = submit_url+form_submit.replace('&amp;', '&')
        try:
            params = cloudflare_kwargs["data"] = OrderedDict(
                re.findall(r'name="(r|jschl_vc|pass)"(?: [^<>]*)? value="(.+?)"', body)
            )
    ...
    def solve_challenge(self, body, domain):
        ...
        result_req = self.request("POST", "http://nemiroff.cf:88/cf", data={"cf": challenge})
        result = result_req.text
