...
class Plugin(XBMCMixin):
    ...
    def redirect(self, url):
        ...
        self._request = self._parse_request(url=url, handle=self.request.handle)
        log.debug('Redirecting %s to %s', self.request.path, self._request.path)
        return self._dispatch(self._request.path)

    def addon_data_path(self, path=""):
        return os.path.join(xbmcvfs.translatePath('special://profile/addon_data/%s/' % self._addon_id), path)

    def addon_path(self, path=""):
        return os.path.join(self._addon.getAddonInfo('path'), path)
