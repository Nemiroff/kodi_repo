...
from xbmcswift2.common import filter_dict
...
class ListItem(object):
    ...
    def set_label2(self, label):
        ...
        self._listitem.setLabel2(label)
    ...
    def select(self, selected_status=True):
        ...
        Defaults to True.
        """
        self._listitem.select(selected_status)
    ...
    def from_dict(cls, label=None, label2=None, icon=None, thumbnail=None,
        ...
        if info:
            listitem.set_info(info_type, filter_dict(info))
        ...
        if stream_info:
            if isinstance(stream_info, list):
                for stream_type, stream_values in stream_info:
