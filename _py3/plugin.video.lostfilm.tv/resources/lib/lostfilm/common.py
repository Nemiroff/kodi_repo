...
from xbmcswift2 import xbmcgui, actions, xbmc, abort_requested, xbmcvfs
...
def get_scraper():
    ...
    return LostFilmScraper(login=plugin.get_setting('login', str),
...
def create_lostfilm_source():
    ...
    try:
        sources.add_video(plugin.get_setting('library-path', str), 'LostFilm.TV', TvDbScraperSettings(), plugin.addon_path("icon.png"))
...
def get_library():
    path = plugin.get_setting('library-path', str)
...
def update_library():
    ...
    if plugin.get_setting('update-xbmc-library', bool):
        if lib.added_medias or lib.created_medias or lib.updated_medias:
            ...
            plugin.update_library('video', plugin.get_setting('library-path', str))
