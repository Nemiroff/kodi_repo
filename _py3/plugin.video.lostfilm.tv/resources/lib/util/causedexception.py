...
if __name__ == '__main__':
    ...
    def library(i):
        if i == 0:
            ...
            try:
                deeplib(i)
            except CausedException as e:
                raise CausedException("lib error two %d" % i, cause=e)
            except Exception as e:  # non-caused exception?
    ...
    try:
        application()
    except CausedException as exc:
        exc.write()
        print("NOW WITH MORE OBVIOUS INDENTATION", file=sys.stderr)
        exc.write(indentation='||  ')
    print("NOW THE DEFAULT HANDLER", file=sys.stderr)
    application()