...
class Torrent:
    ...
    def download_locally(self, path):
        path = os.path.join(path, hashlib.md5(self.url.encode('utf-8')).hexdigest() + ".torrent")
    ...
    def url(self):
        if self._url is None:
            if self.has_file_name():
                self._url = urlparse.urljoin('file:', urllib.request.pathname2url(ensure_str(self.file_name)))
            else:
