...
LOWERCASE_LETTERS = 'abcdefghijklmnopqrstuvwxyzабвгдеёжзийклмнопрстуфхцчшщъыьэюя'
UPPERCASE_LETTERS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ'
...
def save_path(local=False):
    path = plugin.get_setting('save-path', str)
    if path == plugin.get_setting('temp-path', str):
...
def temp_path():
    path = ensure_path_local(plugin.get_setting('temp-path', str))
